...
/**
 * A {@link RuntimeException} generated any time an {@link cminor.messages.errors.Error} needs to be displayed.
 * <p>
...
public class CompilationMessage extends RuntimeException {
    ...
    /**
     * Shared flag that determines whether we should print out a stack trace (only for debugging purposes).
     */
     ...
     *     within the VM's {@link cminor.ast.misc.CompilationUnit} during name checking, we have to make sure these
     *     same constructs are properly removed from the global scope if a future compilation error occurs.
     * </p>
     * @param globalScope {@link SymbolTable} representing the global scope of the {@link cminor.interpreter.VM}.
     */
    public void updateGlobalScope(SymbolTable globalScope) {
        ...
        if(node.isClassNode())
            globalScope.removeName(node.asClassNode().getClassDecl());
        else
     ...
     * @return {@code True} if the error is a {@link cminor.messages.errors.scope.RedeclarationError}, {@code False} otherwise.
     */
